...
class MoeDriver(DenseDriver):
    ...
    def _check_fused(
        ...
        if qualified and not unqualified:
            ...
            bad_rest = (
                check_envelope(m_rest, env) if remaining else ("no non-exception tokens remain")
            )
            if bad_rest is None:
        ...
        for entry in qualified + unqualified:
            t = entry["token"]
            spreads = {
                e: round(self._expert_scale_spread(layer_idx, e), 1) for e in entry["top8_experts"]
            }
            diagnostics.append(
                {
                    **entry,
    ...
    def run(self, json_path: str) -> int:  # noqa: D102 — see module docstring
        ...
        if self.rank == 0:
            ...
            self._c3_exception = {
                ...
                "exception_token_count": sum(len(e["qualified_tokens"]) for e in applications),
